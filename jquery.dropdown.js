// Original from: https://www.jqueryscript.net/form/Searchable-Multi-select-jQuery-Dropdown.html
// Customized by Julia Yan (juliayan.me) - additional features where added to enhance what we can do with the dropdown
...
(function ($) {
  ...
  function objectToSelect(data) {
    ...
    $.each(data, function (index, val) {
      ...
      if (isSelected) {
        ...
        name.push('<span class="dropdown-selected '+type+'">' + val.name + '<i class="del fa fa-times" data-id="' + val.id + '"></i></span>');
        selectAmount++;
      }
      // 判断是否有分组
      if (hasGroup) {
  ...
  function selectToObject(el) {
    ...
    function readOption(key, el) {
      ...
      this.type = $option.attr('class-type');
    }
    ...
    multiChoose: function (event, status) {
      ...
      $.each(_config.data, function (key, item) {
        ...
        if (item.selected) {
          ...
          _dropdown.name.push('<span class="dropdown-selected '+type+'">' + item.name + '<i class="del fa fa-times" data-id="' + item.id + '"></i></span>');
        }
      });
  ...
  Dropdown.prototype = {
    ...
    setSelectListener: function(callback){
        this.config.selectListener = callback;
    },
    notifyDelListener: function(item){
        this.config.delListener(item);
    },
